...
base_path = r'/content/drive/MyDrive/Teknofest_Braincoders/Alt_Kumeler_Augmantasyon'
...
teacher_model_path = "/content/drive/MyDrive/Teknofest_Braincoders/Modeller/inceptionv3/inception_best_model_Fold1.pth"
if not os.path.exists(teacher_model_path):
    raise FileNotFoundError(f"Öğretmen modeli dizini bulunamadı: {teacher_model_path}")
...
class StudentModel(nn.Module):
    def __init__(self):
        ...
        # Get the in_features from the last layer of the original classifier
        in_features = self.model.classifier[-1].in_features
        self.model.classifier = nn.Sequential(
            nn.Linear(in_features, 256), # Use in_features here
            nn.ReLU(),
            nn.Linear(256, 256),
            nn.ReLU(),
            nn.Linear(256, 2)  # İkili sınıflandırma
...
def plot_metrics(train_losses, test_losses, train_accuracies, test_accuracies, fold_name, save_dir=result_path):
    ...
    file_path = os.path.join(save_dir, f"kd__efficientnet_b0_plots_{fold_name}.png")
...
def save_metrics_to_file_txt(fold_name, epoch, train_loss, test_loss, train_acc, test_acc, save_dir=result_path):
    os.makedirs(save_dir, exist_ok=True)  # Klasör yoksa oluştur
    file_path = os.path.join(save_dir, f"kd_efficientnet_b0_losses_{fold_name}.txt")
...
def save_final_metrics_to_file(fold_name, labels, preds, save_dir=result_path):
    ...
    file_path = os.path.join(save_dir, f"kd_efficientnet_b0_metrics_{fold_name}.txt")
...
for fold in folds:
    ...
    model_path = os.path.join(trained_model_path, f"kd_efficientnet_b0_{fold}.pth")
